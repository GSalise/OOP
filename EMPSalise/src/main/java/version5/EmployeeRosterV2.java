 ...
 * @author User
 */
public class EmployeeRosterV2 {
    private ArrayList<Employee> empList;
    
    
    public EmployeeRosterV2(){
    ...
    public void displayAllEmployee(){
        ...
        for(int i = 0; i < this.empList.size(); i++){
            double salary = 0.0;
            if(this.empList.get(i) instanceof Hourly){
                Hourly a = (Hourly)this.empList.get(i);
                salary = a.computeSalary();
            }
            
            if(this.empList.get(i) instanceof PieceWorker){
                PieceWorker a = (PieceWorker)this.empList.get(i);
                salary = a.computeSalary();
            }
            
            if(this.empList.get(i) instanceof Commission && !(this.empList.get(i) instanceof BasedPlusCommission)){
                Commission a = (Commission)this.empList.get(i);
                salary = a.computeSalary();
            }
      
            if(this.empList.get(i) instanceof BasedPlusCommission){
                BasedPlusCommission a = (BasedPlusCommission)this.empList.get(i);
                salary = a.computeSalary();
            }
            
            this.empList.get(i).getClass().getSimpleName();
            String res = String.format("%-6d | %-30s | %-14s | %-14s | %-12.2f | %-12s", this.empList.get(i).getEmpID(), this.empList.get(i).getEmpName(), this.empList.get(i).getHireDate(), this.empList.get(i).getDob(), salary, this.empList.get(i).getClass().getSimpleName());
